...
local resultFile = Path(Content.writablePath, "DoraXRootDisposeTest.result") -- 5
Content:save(resultFile, "running") -- 6
local function fail(message) -- 8
	error("[DoraXRootDisposeTest] " .. message) -- 9
end -- 8
...
root:dispose() -- 32
expect(not host.hasChildren, "dispose should unmount root children") -- 33
value.value = 1 -- 35
Director.systemScheduler:schedule(once(function() -- 37
	expect(renders == 1, "disposed root should not update after signal change") -- 38
	...
	expect(renders == 2, "disposed root should be renderable again") -- 44
	...
	Director.systemScheduler:schedule(once(function() -- 49
		...
		Log("Info", "[DoraXRootDisposeTest] passed") -- 55
	end)) -- 49
end)) -- 37
