...
run_dora cli doctor -p "${PROJECT}" --fix >/dev/null

for file in "${SCRIPT_DIR}"/*.tsx; do
	(
		cd "${PROJECT}"
		run_dora cli build -f "Test/InputManager/${file:t}"
	)
done

for file in "${SCRIPT_DIR}"/*Test.tsx; do
	test_name=${file:t:r}
	marker="${MARKER_ROOT}/${test_name}.result"
	log_file="/tmp/input_manager_${test_name}.run.log"
	rm -f "${marker}" "${log_file}"
	run_dora cli doctor -p "${PROJECT}" --fix >/dev/null
	print "== ${test_name} =="
	(
		cd "${PROJECT}"
		run_dora cli run --entry "Test/InputManager/${test_name}.lua"
	) >"${log_file}" 2>&1
	passed=false
	for _ in {1..100}; do
		if [[ -f "${marker}" ]]; then
