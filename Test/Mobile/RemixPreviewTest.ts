import { App, Content, Path, Size, sleep, thread } from "Dora";
...
thread(() => {
	...
	const host = startMobileRemix({
		entry: {
			...
			title: "轨道花园",
			workDir: Path.getPath(Path.getPath(Content.getFullPath("Test/Mobile"))),
			fileName: Content.getFullPath("Test/Mobile/FeedPreviewTest.lua"),
		},
		onBack: () => undefined,
		onPlay: () => undefined,
	});
