...
OUTPUT=${DORA_3D_ANIMATION_PROFILE_OUTPUT:-/tmp/dora-3d-animation-profile}
...
sleep 2
run_dora cli build -f "${SCRIPT_DIR}/AnimationScaleProfile.ts"
rm -rf "${OUTPUT}" "${STAGE}"
mkdir -p "${OUTPUT}" "${STAGE}/Test/Model3D"
rsync -a "${SCRIPT_DIR}/Assets/" "${STAGE}/Test/Model3D/Assets/"
cp "${SCRIPT_DIR}/AnimationScaleProfile.lua" "${STAGE}/init.lua"
...
if [[ ! -f "${OUTPUT}/result.txt" ]]; then
	print -u2 "Animation scale profile timed out"
	exit 1
fi

cat "${OUTPUT}/result.txt"
grep -q '^ANIMATION_PROFILE_SUMMARY status=PASS' "${OUTPUT}/result.txt"
