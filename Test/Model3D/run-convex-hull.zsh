...
OUTPUT=${DORA_3D_CONVEX_HULL_OUTPUT:-/tmp/dora-3d-convex-hull}
...
if [[ ! -f "${OUTPUT}/result.txt" ]]; then
	print -u2 "Convex hull test timed out"
	exit 1
fi

cat "${OUTPUT}/result.txt"
grep -q '^CONVEX_HULL3D_SUMMARY status=PASS' "${OUTPUT}/result.txt"
