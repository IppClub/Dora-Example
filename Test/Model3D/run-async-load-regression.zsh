...
OUTPUT=${DORA_3D_ASYNC_OUTPUT:-/tmp/dora-3d-async}
...
sleep 2
run_dora cli build -f "${SCRIPT_DIR}/AsyncLoadRegression.ts"
rm -rf "${OUTPUT}" "${STAGE}"
mkdir -p "${OUTPUT}" "${STAGE}/Test/Model3D"
rsync -a "${SCRIPT_DIR}/Assets/" "${STAGE}/Test/Model3D/Assets/"
cp "${SCRIPT_DIR}/AsyncLoadRegression.lua" "${STAGE}/init.lua"
...
if [[ ! -f "${OUTPUT}/result.txt" ]]; then
	print -u2 "Async load regression timed out"
	exit 1
fi

cat "${OUTPUT}/result.txt"
grep -q '^ASYNC_SUMMARY status=PASS' "${OUTPUT}/result.txt"
