...
OUTPUT=${DORA_3D_CONSTRAINT_OUTPUT:-/tmp/dora-3d-constraint}
...
sleep 2
run_dora cli build -f "${SCRIPT_DIR}/Constraint3D.ts"
rm -rf "${OUTPUT}" "${STAGE}"
mkdir -p "${OUTPUT}" "${STAGE}/Test/Model3D"
rsync -a "${SCRIPT_DIR}/Assets/" "${STAGE}/Test/Model3D/Assets/"
cp "${SCRIPT_DIR}/Constraint3D.lua" "${STAGE}/init.lua"
...
if [[ ! -f "${OUTPUT}/result.txt" ]]; then
	print -u2 "Constraint3D test timed out"
	exit 1
fi

cat "${OUTPUT}/result.txt"
grep -q '^CONSTRAINT3D_SUMMARY status=PASS' "${OUTPUT}/result.txt"
