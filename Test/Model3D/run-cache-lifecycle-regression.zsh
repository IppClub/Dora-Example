...
OUTPUT=${DORA_3D_CACHE_OUTPUT:-/tmp/dora-3d-cache}
...
sleep 2
run_dora cli build -f "${SCRIPT_DIR}/CacheLifecycleRegression.ts"
rm -rf "${OUTPUT}" "${STAGE}"
mkdir -p "${OUTPUT}" "${STAGE}/Test/Model3D"
rsync -a "${SCRIPT_DIR}/Assets/" "${STAGE}/Test/Model3D/Assets/"
cp "${SCRIPT_DIR}/CacheLifecycleRegression.lua" "${STAGE}/init.lua"
...
if [[ ! -f "${OUTPUT}/result.txt" ]]; then
	print -u2 "Cache lifecycle regression timed out"
	exit 1
fi

cat "${OUTPUT}/result.txt"
grep -q '^CACHE_SUMMARY status=PASS' "${OUTPUT}/result.txt"
