...
OUTPUT=${DORA_3D_COMPOUND_OUTPUT:-/tmp/dora-3d-compound}
...
sleep 2
run_dora cli build -f "${SCRIPT_DIR}/CompoundShape3D.ts"
rm -rf "${OUTPUT}" "${STAGE}"
mkdir -p "${OUTPUT}" "${STAGE}/Test/Model3D"
rsync -a "${SCRIPT_DIR}/Assets/" "${STAGE}/Test/Model3D/Assets/"
cp "${SCRIPT_DIR}/CompoundShape3D.lua" "${STAGE}/init.lua"
...
if [[ ! -f "${OUTPUT}/result.txt" ]]; then
	print -u2 "Compound shape test timed out"
	exit 1
fi

cat "${OUTPUT}/result.txt"
grep -q '^COMPOUND3D_SUMMARY status=PASS' "${OUTPUT}/result.txt"
