...
OUTPUT=${DORA_3D_RENDERER_PROFILE_OUTPUT:-/tmp/dora-3d-renderer-profile}
...
sleep 2
run_dora cli build -f "${SCRIPT_DIR}/RendererScaleProfile.ts"
rm -rf "${OUTPUT}" "${STAGE}"
mkdir -p "${OUTPUT}" "${STAGE}/Test/Model3D"
rsync -a "${SCRIPT_DIR}/Assets/" "${STAGE}/Test/Model3D/Assets/"
cp "${SCRIPT_DIR}/RendererScaleProfile.lua" "${STAGE}/init.lua"
...
if [[ ! -f "${OUTPUT}/result.txt" ]]; then
	print -u2 "Renderer scale profile timed out"
	exit 1
fi

cat "${OUTPUT}/result.txt"
grep -q '^RENDERER_PROFILE_SUMMARY status=PASS' "${OUTPUT}/result.txt"
