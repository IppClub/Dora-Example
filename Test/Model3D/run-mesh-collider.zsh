...
OUTPUT=${DORA_3D_MESH_COLLIDER_OUTPUT:-/tmp/dora-3d-mesh-collider}
...
if [[ ! -f "${OUTPUT}/result.txt" ]]; then
	print -u2 "Mesh collider test timed out"
	exit 1
fi

cat "${OUTPUT}/result.txt"
grep -q '^MESH_COLLIDER3D_SUMMARY status=PASS' "${OUTPUT}/result.txt"
