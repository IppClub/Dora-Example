...
const fixtureRoot = fileURLToPath(new URL("./Fixtures/PhysicsScene", import.meta.url));
...
async function waitForContent(path, expected, timeoutMs = 20000) {
	...
	do {
		...
		if (result.success) {
			assert(result.content === expected, `unexpected physics status: ${result.content}`);
			return;
		}
		await new Promise(resolve => setTimeout(resolve, 25));
	} while (Date.now() < deadline);
	throw new Error(`timed out waiting for physics status at ${path}`);
}

const status = await post("/status");
assert(status.success && status.writablePath, "Dora status is incomplete");
const root = `${status.writablePath}/.download/love-physics`;
...
try {
	...
	const command = await post("/command", {
		code: `Content\\insertSearchPath 1, ${JSON.stringify(fixtureRoot)}\npackage.loaded.host = nil\nphysicsWorkflow = require "host"\nphysicsWorkflow.run ${JSON.stringify(statusFile)}`,
		log: true,
	});
	assert(command.success, `failed to queue physics workflow: ${command.message ?? ""}`);
	await waitForContent(statusFile, "ghost-manifold=pass ccd=pass");
	console.log("LOVE_PHYSICS_WORKFLOW_PASS ghost-manifold=pass ccd=pass");
} finally {
	await post("/command", {
