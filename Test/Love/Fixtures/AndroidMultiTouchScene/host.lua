...
function workflow.run(readyFile, statusFile)
	local node = assert(LoveNode("main.lua"), "failed to create Android multi-touch LoveNode")
	...
	Director.systemScheduler:schedule(function()
		...
		elseif node.running then
			result = "failed: Android multi-touch timed out"
		else
			result = "platform=android touches=2 move=both release=ordered source=inputreader content=pass"
		end
		assert(Content:save(statusFile, result))
		node:removeFromParent(true)
		package.loaded.host = nil
		print("HOST_LOVE_ANDROID_MULTITOUCH_DONE", result, frames)
		return true
	end)
end
