...
function workflow.run(readyFile, statusFile)
	local node = assert(LoveNode("main.lua"), "failed to create Android system-input LoveNode")
	Director.entry:addChild(node)
	local frames = 0
	local ready = false
	Director.systemScheduler:schedule(function()
		frames = frames + 1
		if node.running and not ready and frames >= 5 then
		...
		elseif node.running then
			result = "failed: Android system input timed out"
		else
			result = "platform=android touch=press+move+release key=a source=os content=pass"
		end
		assert(Content:save(statusFile, result))
		node:removeFromParent(true)
		package.loaded.host = nil
		print("HOST_LOVE_ANDROID_SYSTEM_INPUT_DONE", result, frames)
		return true
	end)
end
