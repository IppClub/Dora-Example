...
function workflow.run(statusFile)
	local node = assert(LoveNode("main.lua"), "failed to create draw Transform LoveNode")
	...
	Director.systemScheduler:schedule(function()
		...
		assert(not node.running, "draw Transform LoveNode did not quit within 120 frames")
		assert(node.lastError == "", node.lastError)
		assert(Content:save(statusFile,
			"texture_transform=pass quad_transform=pass composition=pass origin=pass"))
		node:removeFromParent(true)
		package.loaded.host = nil
		print("HOST_LOVE_DRAW_TRANSFORM_PASS", frames)
		return true
	end)
end
