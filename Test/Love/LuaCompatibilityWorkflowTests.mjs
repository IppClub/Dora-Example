...
const fixtureRoot = fileURLToPath(new URL("./Fixtures/LuaCompatibilityScene", import.meta.url));
...
async function waitForContent(path, expected, timeoutMs = 20000) {
	...
	do {
		...
		if (result.success) {
			assert(result.content === expected, `unexpected Lua compatibility status: ${result.content}`);
			return;
		}
		await new Promise(resolve => setTimeout(resolve, 25));
	} while (Date.now() < deadline);
	throw new Error(`timed out waiting for Lua compatibility status at ${path}`);
}

const status = await post("/status");
assert(status.success && status.writablePath, "Dora status is incomplete");
assert(status.wsConnectionCount === 1,
	`Lua compatibility workflow requires exactly one Web IDE compiler, got ${status.wsConnectionCount}`);
const root = `${status.writablePath}/.download/love-lua55-compat`;
...
try {
	...
	const command = await post("/command", {
		code: `Content\\insertSearchPath 1, ${JSON.stringify(fixtureRoot)}\npackage.loaded.host = nil\nluaCompatibilityWorkflow = require "host"\nluaCompatibilityWorkflow.run ${JSON.stringify(statusFile)}`,
		log: true,
	});
