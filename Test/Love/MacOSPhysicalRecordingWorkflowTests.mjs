...
const workflowKey = `loveMacOSPhysicalRecording_${crypto.randomUUID().replaceAll("-", "")}`;
...
async function waitForContent(path, timeoutMs) {
	...
	throw new Error(`timed out waiting for macOS physical recording status at ${path}`);
}

const status = await post("/status");
assert(status.success && status.platform === "macOS" && status.writablePath,
	`physical recording workflow requires macOS Dora, got ${status.platform ?? "unknown"}`);

const root = `${status.writablePath}/.download/love-macos-physical-recording-${workflowKey}`;
...
try {
	...
	for (const filename of files) {
		await create(`${root}/${filename}`,
			readFileSync(`${testsRoot}/Fixtures/PhysicalRecordingScene/${filename}`, "utf8"));
	}
	...
	assert(command.success, `failed to queue physical recording workflow: ${command.message ?? ""}`);
	initialized = true;
	await waitForContent(readyFile, 15000);
	console.log("LOVE_MACOS_PHYSICAL_RECORDING_READY press-space-or-click-then-allow-microphone");
	const result = await waitForContent(statusFile, 45000);
	assert(result.startsWith("platform=macOS "), `physical recording failed: ${result}`);
	assert(/ samples=\d+ format=16000\/16\/1 waveform=pass /.test(result),
		`physical recording result is incomplete: ${result}`);
	console.log(`LOVE_MACOS_PHYSICAL_RECORDING_WORKFLOW_PASS ${result}`);
} finally {
	if (initialized) {
		await post("/command", {
