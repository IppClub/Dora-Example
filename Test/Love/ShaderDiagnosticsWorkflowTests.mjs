...
const fixtureRoot = fileURLToPath(new URL("./Fixtures/ShaderDiagnosticsScene", import.meta.url));
const expected = "line-map=pass warnings=pass state=pass";
	...
	do {
		...
		if (result.success) {
			assert(result.content === expected,
				`unexpected Shader diagnostics status: ${result.content}`);
			return;
		}
		await new Promise(resolve => setTimeout(resolve, 25));
	} while (Date.now() < deadline);
	throw new Error(`timed out waiting for Shader diagnostics status at ${path}`);
}

const status = await post("/status");
assert(status.success && status.writablePath, "Dora status is incomplete");
const root = `${status.writablePath}/.download/love-shader-diagnostics`;
...
try {
	...
	const command = await post("/command", {
		code: `Content\\insertSearchPath 1, ${JSON.stringify(fixtureRoot)}\npackage.loaded.host = nil\nshaderDiagnosticsWorkflow = require "host"\nshaderDiagnosticsWorkflow.run ${JSON.stringify(statusFile)}`,
		log: true,
	});
	assert(command.success, `failed to queue Shader diagnostics workflow: ${command.message ?? ""}`);
	await waitForContent(statusFile);
	console.log(`LOVE_SHADER_DIAGNOSTICS_WORKFLOW_PASS ${expected}`);
} finally {
	await post("/command", {
